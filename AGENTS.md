...
- Seguridad/datos: no loguear secretos; no exponer JWT (`session` cookie con `ownerId` en `src/lib/auth.ts`); manejar errores de Supabase/Resend con mensajes seguros; respetar 401 → redirigir a `/login`.
